...
class GrandChecker:
    ...
    def find_out_language(self, content):
        ...
        results = GrandChecker.sort_language(results)
    ...
    def sort_language(item):
        ...
        for c in item:
            if piv == {}:
                piv = c
        return GrandChecker.sort_language([c for c in item if c['probability'] > piv['probability']]) + [piv]  + GrandChecker.sort_language([c for c in item if c['probability'] < piv['probability']])

    """@staticmethod
    def sort_language (item):
        if int(item['probability']) > int(item["probability"]):
            return -1
        elif int(item["probability"]) == int(item["probability"]):
            return 0
        else:
            return 1"""
